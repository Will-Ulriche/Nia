...
export function SubjectManagerModal({ isOpen, onClose }: SubjectManagerModalProps) {
  ...
  const loadClassSubjects = async (classId: string) => {
    ...
    try {
      setIsLoading(true);
      const cs = await SubjectService.listClassSubjects(school.id, classId, selectedYear.id);
      setClassSubjects(cs);
    } catch (err: any) {
      setError(err.message);
    } finally {
  ...
  const handleDeleteLibrarySubject = async (id: string, name: string) => {
    if (window.confirm(`Supprimer dÃ©finitivement la matiÃ¨re globale "${name}" ?`)) {
      try {
  ...
  const handleAssignSubject = async () => {
    if (!school || !selectedYear || !selectedClassId || !selectedLibrarySubjectId) {
      setError("Veuillez sÃ©lectionner une matiÃ¨re de la bibliothÃ¨que.");
      return;
    }
    try {
      setError(null);
      if (classSubjects.some(cs => cs.subject_id === selectedLibrarySubjectId)) {
        setError("Cette matiÃ¨re est dÃ©jÃ  associÃ©e Ã  cette classe.");
        return;
      }
  ...
  const handleDeleteClassSubject = async (id: string, name: string) => {
    if (window.confirm(`Retirer la matiÃ¨re ${name} de cette classe ?`)) {
      try {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      alert(`ModÃ¨le "${tmpl.name}" sauvegardÃ© !`);
      setNewTemplateName('');
  ...
  const handleApplyTemplate = async () => {
    if (!school || !selectedYear || !selectedClassId || !applyTemplateId) return;
    try {
      await SubjectService.applyTemplateToClass(school.id, selectedYear.id, selectedClassId, applyTemplateId);
      await loadClassSubjects(selectedClassId);
      setApplyTemplateId('');
      setShowTemplateDropdown(false);
    } catch (err: any) {
  ...
  const handleDeleteTemplate = async () => {
    ...
    if (!tmpl) return;
    if (window.confirm(`Supprimer dÃ©finitivement le modÃ¨le "${tmpl.name}" ?`)) {
      try {
        ...
        <div style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', padding: '0 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0 10px 0' }}>
            <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 700, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <i className="ti ti-books" style={{ color: '#10b981' }} /> Gestionnaire des MatiÃ¨res
            </h2>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto', background: '#fff', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ padding: '20px', borderBottom: '1px solid #e2e8f0', background: '#fcfcfc', display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
                <div style={{ flex: 2, minWidth: 0 }}>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>Nouvelle matiÃ¨re</label>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  <tr>
                    <th style={{ padding: '12px 20px', fontSize: '13px', fontWeight: 600, color: '#475569' }}>Nom de la matiÃ¨re</th>
                ...
                <tbody>
                  {librarySubjects.length === 0 ? (
                    <tr><td colSpan={3} style={{ padding: '32px', textAlign: 'center', color: '#94a3b8' }}>La bibliothÃ¨que est vide.</td></tr>
                  ) : (
                    librarySubjects.map(s => (
                      <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '16px 20px', fontSize: '14px', color: '#1e293b', fontWeight: 600 }}>{s.name}</td>
                        <td style={{ padding: '16px 20px', fontSize: '14px', color: '#64748b' }}>{s.code || 'â€”'}</td>
                        <td style={{ padding: '16px 20px', textAlign: 'right' }}>
                          <button onClick={() => handleDeleteLibrarySubject(s.id, s.name)} title="Supprimer de la bibliothÃ¨que" style={{ background: '#fef2f2', color: '#ef4444', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer' }}><i className="ti ti-trash" /></button>
                        </td>
                      </tr>
                    ))
                  )}
            ...
            <div style={{ flex: 1, padding: '32px', overflowY: 'auto', background: '#fff', position: 'relative' }}>
              {!selectedClassId ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#94a3b8', flexDirection: 'column' }}>
                  ...
                  SÃ©lectionnez une classe Ã  gauche
                </div>
              ) : isLoading ? (
                ...
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', position: 'relative' }}>
                      ...
                      >
                        <i className="ti ti-template" /> ModÃ¨les <i className="ti ti-chevron-down" style={{ fontSize: '14px', opacity: 0.6 }} />
                      </button>
                      ...
                      {showTemplateDropdown && (
                        <div style={{ position: 'absolute', top: '100%', right: '140px', marginTop: '8px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)', padding: '16px', width: '320px', zIndex: 10 }}>
                          <h4 style={{ margin: '0 0 12px 0', fontSize: '13px', color: '#475569', textTransform: 'uppercase' }}>Appliquer un modÃ¨le</h4>
                          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                            ...
                            <button onClick={handleDeleteTemplate} title="Supprimer ce modÃ¨le" disabled={!applyTemplateId} style={{ background: applyTemplateId ? '#fef2f2' : '#f1f5f9', color: applyTemplateId ? '#ef4444' : '#cbd5e1', border: 'none', padding: '8px 12px', borderRadius: '8px', cursor: applyTemplateId ? 'pointer' : 'not-allowed' }}><i className="ti ti-trash" /></button>
                          </div>
                          
                          <h4 style={{ margin: '0 0 12px 0', fontSize: '13px', color: '#475569', textTransform: 'uppercase' }}>Sauvegarder l'actuel</h4>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <input type="text" value={newTemplateName} onChange={e => setNewTemplateName(e.target.value)} placeholder="Nom du modÃ¨le" style={{ flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid #cbd5e1' }} />
                      ...
                      >
                        <i className="ti ti-plus" /> Ajouter une matiÃ¨re
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={{ border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                      <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                        <tr>
                          <th style={{ padding: '4px 10px', fontSize: '12px', fontWeight: 600, color: '#475569', textTransform: 'uppercase' }}>MatiÃ¨re</th>
                      ...
                      <tbody>
                        {classSubjects.length === 0 ? (
                          <tr><td colSpan={5} style={{ padding: '30px', textAlign: 'center', color: '#94a3b8' }}>Aucune matiÃ¨re pour le moment.</td></tr>
                        ) : (
                  ...
                  {showAddSlideover && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <h3 style={{ margin: 0, fontSize: '18px', color: '#1e293b' }}>Ajouter Ã  {selectedClass?.name}</h3>
                      ...
                      <div style={{ padding: '24px', flex: 1, overflowY: 'auto' }}>
                        <div style={{ marginBottom: '20px' }}>
                          <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: '#334155', marginBottom: '8px' }}>MatiÃ¨re (BibliothÃ¨que)</label>
                          ...
                          {librarySubjects.length === 0 && <p style={{ fontSize: '12px', color: '#ef4444', marginTop: '6px' }}>La bibliothÃ¨que est vide. Allez dans l'onglet BibliothÃ¨que pour en crÃ©er.</p>}
                        </div>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                          <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: '#334155', marginBottom: '8px' }}>Type de matiÃ¨re</label>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '32px' }}>
                          ...
                          <label htmlFor="is_mandatory" style={{ fontSize: '14px', fontWeight: 500, color: '#334155', cursor: 'pointer' }}>MatiÃ¨re obligatoire</label>
                        </div>
